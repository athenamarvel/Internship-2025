...
# Recode les noms AVANT le fit du modèle (important !)
sub_data$part <- dplyr::recode(
  sub_data$part,
  "Before" = "Avant",
  "After" = "Après"
)

# Facteurs avec bons niveaux
sub_data$part <- factor(sub_data$part, levels = c("Avant", "Après"))
sub_data$fish <- factor(sub_data$fish)

# ❗️ (Re)fitter ton modèle ici avec sub_data modifié
# hmm_tmb <- ...

# Créer newdata avec les bons niveaux
newdata <- expand.grid(
  part = levels(sub_data$part),   # Avant, Après
  fish = levels(sub_data$fish)
)
...
# Construire pred_data (avec colonnes renommées en français)
pred_data <- lapply(seq_along(delta_preds$post), function(.draw) {
  ...
  do.call("rbind", args = _) |>
  tidyr::pivot_longer(cols = dplyr::contains("state"),
                      names_to = "state",
                      values_to = "probabilité") |>
  dplyr::group_by(part, state, fish) |>
  ggdist::median_qi(.exclude = c(".chain", ".iteration", ".draw", ".row")) |>
  dplyr::ungroup() |>
  dplyr::rename(partie = part)

# Construire marg_pred (marginal)
marg_pred <- lapply(seq_along(delta_preds$post), function(.draw) {
  ...
  tidyr::pivot_longer(cols = dplyr::contains("state"),
                      names_to = "state",
                      values_to = "probabilité") |>
  dplyr::reframe(probabilité = mean(probabilité), .by = c(part, state, .draw)) |>
  dplyr::group_by(part, state) |>
  ggdist::median_qi(.exclude = c(".chain", ".iteration", ".draw", ".row")) |>
  dplyr::ungroup() |>
  dplyr::rename(partie = part)

# Étiquettes traduites pour les états
state_labels <- c(
  "state 1" = "Repoussé",
  "state 2" = "Indifférent",
  "state 3" = "Attiré"
...
# Plot final
ggplot(data = pred_data, aes(x = partie, y = probabilité)) +
  geom_point(size = 2, alpha = 0.5, position = position_dodge(width = 0.25)) +
  geom_line(aes(group = interaction(fish, state)),
            alpha = 0.25,
            position = position_dodge(width = 0.25)) +
  geom_pointrange(data = marg_pred,
                  aes(ymin = .lower, ymax = .upper, color = partie),
                  size = 1,
                  position = position_dodge(width = 0.25)) +
